...
def volatility_of_volatility_features(u: pd.DataFrame) -> dict:
    ...
    _add_diff_ratio_feats(feats, f'rolling_std_w{window}_mean', mean1, mean2)
...
def fin_domin_features(u: pd.DataFrame) -> dict:
    ...
    def calculate_bollinger_bands(prices, window=20, num_std=2):
        ...
        rolling_mean = pd.Series(prices).rolling(window=window).mean().iloc[-1]
        rolling_std = pd.Series(prices).rolling(window=window).std().iloc[-1]
        ...
        return bb_width, bb_position, rolling_std / rolling_mean if rolling_mean != 0 else 0
    ...
    def calculate_volatility_features(prices):
        ...
        # 确保价格为正值，避免log计算警告
        prices_positive = np.abs(prices) + 1e-8
        returns = np.diff(np.log(prices_positive))
        
        # 历史波动率
        historical_vol = np.std(returns) * np.sqrt(252)  # 年化波动率
        ...
        return historical_vol, downside_vol, max_drawdown, var_95
    ...
    def calculate_momentum_features(prices):
        ...
        momentum_10 = (prices[-1] - prices[-11]) / prices[-11] if len(prices) >= 11 and prices[-11] != 0 else 0
        
        # 动量振荡器
        if len(prices) >= 20:
            momentum_osc = (prices[-1] - np.min(prices[-20:])) / (np.max(prices[-20:]) - np.min(prices[-20:])) if (np.max(prices[-20:]) - np.min(prices[-20:])) != 0 else 0.5
        else:
            momentum_osc = 0.5
            
        return momentum_5, momentum_10, momentum_osc
    ...
    def calculate_support_resistance(prices):
        ...
        # 距离阻力位的比例
        resistance_ratio = (recent_high - current_price) / recent_high if recent_high != 0 else 0
        # 距离支撑位的比例
        support_ratio = (current_price - recent_low) / recent_low if recent_low != 0 else 0
        
        return resistance_ratio, support_ratio
    
    # 计算各段特征
    for segment, data in [('left', s1), ('right', s2), ('whole', s_whole)]:
        if len(data) == 0:
            continue
            
        # RSI
        rsi = calculate_rsi(data)
        feats[f'fin_rsi_{segment}'] = rsi
        
        # 布林带
        bb_width, bb_pos, bb_volatility = calculate_bollinger_bands(data)
        feats[f'fin_bb_width_{segment}'] = bb_width
        feats[f'fin_bb_position_{segment}'] = bb_pos
        feats[f'fin_bb_volatility_{segment}'] = bb_volatility
        
        # MACD
        macd, signal, histogram = calculate_macd(data)
        feats[f'fin_macd_{segment}'] = macd
        feats[f'fin_macd_signal_{segment}'] = signal
        feats[f'fin_macd_histogram_{segment}'] = histogram
        
        # 波动率特征
        hist_vol, down_vol, max_dd, var_95 = calculate_volatility_features(data)
        feats[f'fin_historical_vol_{segment}'] = hist_vol
        feats[f'fin_downside_vol_{segment}'] = down_vol
        feats[f'fin_max_drawdown_{segment}'] = max_dd
        feats[f'fin_var_95_{segment}'] = var_95
        
        # 动量特征
        mom_5, mom_10, mom_osc = calculate_momentum_features(data)
        feats[f'fin_momentum_5_{segment}'] = mom_5
        feats[f'fin_momentum_10_{segment}'] = mom_10
        feats[f'fin_momentum_osc_{segment}'] = mom_osc
        
        # 支撑阻力
        resist_ratio, support_ratio = calculate_support_resistance(data)
        feats[f'fin_resistance_ratio_{segment}'] = resist_ratio
        feats[f'fin_support_ratio_{segment}'] = support_ratio
    
    # 添加分段差异特征
    # for base_name in ['fin_rsi', 'fin_bb_width', 'fin_bb_position', 'fin_macd', 'fin_historical_vol', 'fin_momentum_5']:
    #     if f'{base_name}_left' in feats and f'{base_name}_right' in feats:
    #         _add_diff_ratio_feats(feats, base_name, feats[f'{base_name}_left'], feats[f'{base_name}_right'])
            # if f'{base_name}_whole' in feats:
...
def bio_domin_features(u: pd.DataFrame) -> dict:
    ...
    def calculate_hrv_features(signal):
        """心率变异性(HRV)相关特征"""
        if len(signal) < 10:
        ...
        # pNN50: 相邻RR间期差值>50ms的百分比 (这里用标准差作为阈值)
        threshold = np.std(rr_intervals) * 0.5
        pnn50 = np.mean(np.abs(np.diff(rr_intervals)) > threshold) * 100 if len(rr_intervals) > 1 else 0
        ...
        triangular_index = len(rr_intervals) / np.max(hist) if np.max(hist) > 0 else 0
        
        # HRV三角指数
        hrv_index = len(rr_intervals) / (2 * np.max(hist)) if np.max(hist) > 0 else 0
        
        return rmssd, pnn50, sdnn, triangular_index, hrv_index
    ...
    def calculate_eeg_features(signal):
        ...
        freqs = np.fft.fftfreq(len(signal), 1.0)
        ...
        if total_power > 0:
            delta_power = np.sum(power_spectrum[1:4]) / total_power
            theta_power = np.sum(power_spectrum[4:8]) / total_power
            alpha_power = np.sum(power_spectrum[8:13]) / total_power
            beta_power = np.sum(power_spectrum[13:min(30, len(power_spectrum)//2)]) / total_power
        else:
            delta_power = theta_power = alpha_power = beta_power = 0.25
        
        return delta_power, theta_power, alpha_power, beta_power
    
    def calculate_emg_features(signal):
        """肌电图(EMG)相关特征"""
        if len(signal) < 5:
            return 0.0, 0.0, 0.0, 0.0
        ...
        return rms, mav, zcr, wl
    
    def calculate_respiratory_features(signal):
        """呼吸信号特征"""
        if len(signal) < 20:
            return 0.0, 0.0, 0.0, 0.0
        ...
        respiratory_rate = freqs[dominant_freq_idx] * 60  # 转换为每分钟次数
        
        # 呼吸深度变异性
        breath_depth_var = np.var(signal)
        
        # 呼吸不规则性
        breath_irregularity = np.std(np.diff(signal))
        
        # 呼吸暂停检测 (低幅度区域)
        threshold = np.mean(np.abs(signal)) * 0.1
        apnea_ratio = np.mean(np.abs(signal) < threshold)
        
        return abs(respiratory_rate), breath_depth_var, breath_irregularity, apnea_ratio
    
    def calculate_circadian_features(signal):
        """昼夜节律特征"""
        if len(signal) < 24:
            return 0.0, 0.0, 0.0
        
        # 24小时周期性检测
        period_24h = len(signal) // 24 if len(signal) >= 24 else 1
        
        # 昼夜节律强度
        if period_24h > 1:
            reshaped = signal[:period_24h * 24].reshape(-1, 24)
            circadian_strength = np.var(np.mean(reshaped, axis=0))
            
            # 峰值时间
            daily_pattern = np.mean(reshaped, axis=0)
            peak_time = np.argmax(daily_pattern)
            
            # 昼夜比率
            day_hours = daily_pattern[6:18]  # 6AM-6PM
            night_hours = np.concatenate([daily_pattern[:6], daily_pattern[18:]])  # 6PM-6AM
            day_night_ratio = np.mean(day_hours) / (np.mean(night_hours) + 1e-8)
        else:
            circadian_strength = np.var(signal)
            peak_time = np.argmax(signal)
            day_night_ratio = 1.0
        
        return circadian_strength, peak_time, day_night_ratio
    ...
    for segment, data in [('left', s1), ('right', s2), ('whole', s_whole)]:
        ...
        # HRV特征
        rmssd, pnn50, sdnn, tri_idx, hrv_idx = calculate_hrv_features(data)
        feats[f'bio_hrv_rmssd_{segment}'] = rmssd
        feats[f'bio_hrv_pnn50_{segment}'] = pnn50
        feats[f'bio_hrv_sdnn_{segment}'] = sdnn
        feats[f'bio_hrv_triangular_index_{segment}'] = tri_idx
        feats[f'bio_hrv_index_{segment}'] = hrv_idx
        
        # EEG特征
        delta, theta, alpha, beta = calculate_eeg_features(data)
        feats[f'bio_eeg_delta_{segment}'] = delta
        feats[f'bio_eeg_theta_{segment}'] = theta
        feats[f'bio_eeg_alpha_{segment}'] = alpha
        feats[f'bio_eeg_beta_{segment}'] = beta
        
        # EMG特征
        rms, mav, zcr, wl = calculate_emg_features(data)
        feats[f'bio_emg_rms_{segment}'] = rms
        feats[f'bio_emg_mav_{segment}'] = mav
        feats[f'bio_emg_zcr_{segment}'] = zcr
        feats[f'bio_emg_wl_{segment}'] = wl
        
        # 呼吸特征
        resp_rate, depth_var, irregularity, apnea = calculate_respiratory_features(data)
        feats[f'bio_respiratory_rate_{segment}'] = resp_rate
        feats[f'bio_breath_depth_var_{segment}'] = depth_var
        feats[f'bio_breath_irregularity_{segment}'] = irregularity
        feats[f'bio_apnea_ratio_{segment}'] = apnea
        
        # 昼夜节律特征
        circ_strength, peak_time, day_night_ratio = calculate_circadian_features(data)
        feats[f'bio_circadian_strength_{segment}'] = circ_strength
        feats[f'bio_peak_time_{segment}'] = peak_time
        feats[f'bio_day_night_ratio_{segment}'] = day_night_ratio
    
    # # 添加分段差异特征
    # for base_name in ['bio_hrv_rmssd', 'bio_hrv_sdnn', 'bio_eeg_alpha', 'bio_emg_rms', 'bio_respiratory_rate', 'bio_circadian_strength']:
    #     if f'{base_name}_left' in feats and f'{base_name}_right' in feats:
    #         _add_diff_ratio_feats(feats, base_name, feats[f'{base_name}_left'], feats[f'{base_name}_right'])
    #         if f'{base_name}_whole' in feats:
    #             _add_contribution_ratio_feats(feats, base_name, feats[f'{base_name}_left'], feats[f'{base_name}_right'], feats[f'{base_name}_whole'])
